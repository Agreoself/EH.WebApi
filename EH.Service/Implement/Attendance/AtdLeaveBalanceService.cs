...
namespace EH.Service.Implement.Attendance
{
    public class AtdLeaveBalanceService : BaseService<Atd_LeaveBalance>, IAtdLeaveBalanceService, ITransient
    {
        ...
        public AtdLeaveBalanceService(IAtdLeaveFormRepository formRepository, IAtdLeaveSettingRepository settingRepository, IAtdLeaveBalanceRepository repository, ISysUsersRepository userRepository, LogHelper logHelper) : base(repository, logHelper)
        {
        ...
        public bool CalculateAnnualAndSick(List<string> userIds)
        {
            ...
            try
            {
                foreach (var userId in userIds)
                {
                    ...
                    var annualBalance = repository.FirstOrDefault(i => i.UserId == user.UserName && i.LeaveType == "annual" && i.Year == DateTime.Now.Year);
                    ...
                    var sickBalance = repository.FirstOrDefault(i => i.UserId == user.UserName && i.LeaveType == "sick" && i.Year == DateTime.Now.Year);
                    ...
                    if (annualBalance == null)
                    {
                        Atd_LeaveBalance atd_LeaveBalance = new()
                        {
                            ...
                            Overdue = (preAnnualBalance == null ? 0 : preAnnualBalance.Available),
                            Remark = $"{DateTime.Now.Year} annual",
                        };
                    ...
                    if (sickBalance == null)
                    {
                        Atd_LeaveBalance atd_LeaveBalance = new()
                        {
                            ...
                            Used = Convert.ToDecimal(0),
                            Locked = Convert.ToDecimal(0),
                            Overdue = Convert.ToDecimal(0),
                            Remark = $"{DateTime.Now.Year} sick",
                        };
        ...
        public bool CalculatePersonal(List<string> userIds)
        {
            try
            {
                ...
                foreach (var userId in userIds)
                {
                    ...
                    var personalBalance = repository.FirstOrDefault(i => i.UserId == user.UserName && i.LeaveType == "personal" && i.Year == DateTime.Now.Year);
                    ...
                    if (personalBalance == null)
                    {
                        Atd_LeaveBalance atd_LeaveBalance = new Atd_LeaveBalance()
                        {
                            ...
                            Locked = Convert.ToDecimal(0),
                            Overdue = Convert.ToDecimal(0),
                            Remark = $"{DateTime.Now.Year} personal",
                        };
        ...
        public bool CalculateParentalAndBreastfeeding(string userId, DateTime bornDate)
        {
            try
            {
                ...
                List<Atd_LeaveBalance> addLbs = new List<Atd_LeaveBalance>();
                List<Atd_LeaveBalance> updateLbs = new List<Atd_LeaveBalance>();
                for (int i = 0; i < 4; i++)
                {
                    Atd_LeaveBalance atd_LeaveBalance = new Atd_LeaveBalance()
                    {
